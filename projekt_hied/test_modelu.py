...
# KONFIGURACJA
# ==========================================
MODEL_PATH = "najlepszy_model_oko.pth"
# Upewnij się, że masz to zdjęcie na dysku!
TEST_IMAGE_PATH = "./dataset/test/nowe_oko.png"

# Mapowanie kolorów (RGB) dla naszych klas:
# 0: Tło (czarne), 1: Źrenica (czerwony), 2: Tęczówka (zielony), 3: Twardówka (niebieski), 4: Skóra (żółty)
COLORS = np.array([
    [0, 0, 0],  # 0 - Tło
...
def test_model():
    # Model był trenowany/ostatnio używany na CPU, więc ładujemy na CPU
    device = torch.device("cpu")
    print("Ładowanie modelu...")

    # Inicjalizacja pustego modelu o tej samej architekturze
    model = smp.Unet(
    ...
    # Wczytanie Twoich wag z pliku .pth
    ...
    # Transformacje MUSZĄ być dokładnie takie same jak w treningu
    transform = A.Compose([
    ...
    with torch.no_grad():
        ...
        # Wybieramy klasę (0-4), która dostała największe prawdopodobieństwo dla każdego piksela
    ...
    # Tworzenie "nakładki" z przezroczystością (alpha)
    alpha = 0.5
    ...
    plt.subplot(1, 3, 2)
    plt.title("Wygenerowana Maska (AI)")
    plt.imshow(color_mask)
    plt.axis("off")

    plt.subplot(1, 3, 3)
    plt.title("Nałożenie (Overlay)")
    plt.imshow(overlay)
    plt.axis("off")
